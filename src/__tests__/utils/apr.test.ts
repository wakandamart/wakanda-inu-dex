...
describe('getFarmApr', () => {
  it(`returns null when parameters are missing`, () => {
    const { wkdRewardsApr, lpRewardsApr } = getFarmApr(null, null, null, null, 40)
    expect(wkdRewardsApr).toBeNull()
    expect(lpRewardsApr).toEqual(0)
  })
  it(`returns null when APR is infinite`, () => {
    const { wkdRewardsApr, lpRewardsApr } = getFarmApr(BIG_ZERO, BIG_ZERO, BIG_ZERO, '', 40)
    expect(wkdRewardsApr).toBeNull()
    expect(lpRewardsApr).toEqual(0)
  })
  it(`get the correct pool APR`, () => {
    const { wkdRewardsApr, lpRewardsApr } = getFarmApr(BIG_TEN, new BigNumber(1), new BigNumber(100000), '', 40)
    expect(wkdRewardsApr).toEqual(4204800)
    expect(lpRewardsApr).toEqual(0)
  })
